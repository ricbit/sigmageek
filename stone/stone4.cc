...
Grid read_input() {
    ...
    for (int j = 0; j < 10; j++) {
        for (int i = 0; i < 10; i++) {
            temp_grid[(2300 + j) * msize + 2300 + i] = puzzle[j][i] == '1';
        }
    }
...
int score(const Grid& grid, const Position& pos, const Position &goal) {
    return -(pos.level + score_num * distance(pos, goal) / score_den);
}
...
string find_best_distance(const vector<Position>& positions, const Position& goal) {
    auto best = min_element(positions.begin(), positions.end(),
        [&goal](const Position& a, const Position& b) {
            return distance(a, goal) < distance(b, goal);
        });
    cerr << distance(*best, goal) << endl;
    return path(positions, std::distance(positions.begin(), best));
}
...
string search(const Grid& grid) {
    ...
    int steps = 0;
    int best_distance = grid.sx * grid.sy;
    string best_path;
    while (!next.empty()) {
        ...
        int current_distance = abs(pos.y - grid.end.y) + abs(pos.x - grid.end.x);
        if (current_distance < best_distance) {
            best_distance = current_distance;
            best_path = path(trie.positions, current.position);
            cerr << best_distance << "\n";
            cerr << best_path << "\n";
        }
        if (++steps % 100000 == 0) {
        ...
        if (pos.y >= grid.sy or pos.x >= grid.sx) {
            cerr << pos.y << " " << pos.x << "\n";
        }
